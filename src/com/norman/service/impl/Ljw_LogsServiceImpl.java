...
public class Ljw_LogsServiceImpl extends BaseDao implements Ljw_LogsService {
    ...
    @Override
    public JSONArray getMyEmpRepairData(HttpServletRequest request,int depId, int page, int limit) {
        JSONArray result = new JSONArray();
        List<EquipmentRepairVo> data = listByHql("from EquipmentRepairVo where Student="+depId);
        //遍历结果集
    ...
    @Override
    public int getMyEmpRepairSize(int empId) {
        List list = listByHql("FROM EquipmentRepairVo where Student = "+empId);
        return list.size();
    }
}
